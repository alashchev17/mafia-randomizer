...
describe('GameDesk', () => {
  ...
  it('correctly identifies mafia and innocent players', () => {
    ...
    // Check mafia roles
    const mafiaCards = screen.getAllByTestId(/game-desk-card-/).filter((card) => card.getAttribute('data-player-is-mafia') === 'true')
    expect(mafiaCards.length).toBe(2) // Should be Дон and Мафия

    // Check innocent roles
    const innocentCards = screen.getAllByTestId(/game-desk-card-/).filter((card) => card.getAttribute('data-player-is-mafia') === 'false')
